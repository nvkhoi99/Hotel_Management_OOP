...
public class OrderForm extends JPanel {
    ...
    private ArrayList<RoomForOrdering> roomForOrderings;
    private final OrderBLL orderBLL = new OrderBLL(this);
    private RoomTypePanel[] roomTypePanels;
    private RoomToggle[] roomButtons;
    private static final DecimalFormat curencyFormat = new DecimalFormat("#,##0 VNĐ");
    ...
    public OrderForm() {
        this.customer = null;
        initComponents();
        RoomTypePanel.setOrderBLL(orderBLL);
        initRoomTypeList();
        initRoomList();
        //initInAndOut();
    ...
    private void initRoomList() {
        roomForOrderings = orderBLL.getAllRoom();
    ...
    private void sortRoom() {
        for (RoomTypePanel typePanel : roomTypePanels) {
            ...
            ArrayList<RoomToggle> roomByType = orderBLL.getRoomByRoomType(roomButtons, maloaiphong);
            for (RoomToggle temp : roomByType) {
                typePanel.addRoomToChoose(temp);
                temp.setTypePanel(typePanel);
                temp.getRoom().setDongiadat(typePanel.getRoomType().getDongia());
            }
        }
    }
    ...
    private void initInAndOut() {
        Calendar calendar = Calendar.getInstance();
        String homnay = orderBLL.dateFormat.format(calendar.getTime());
        ngaynhan.setText(homnay);
        calendar.add(Calendar.DATE, 1);
        String ngaymai = orderBLL.dateFormat.format(calendar.getTime());
    ...
    private void checkOrder() {
        ArrayList<Integer> orderedRooms = orderBLL.checkOrder(ngaynhan.getText(), ngaytra.getText());
        for (RoomToggle i : roomButtons) {
            if (i.isSelected()) {
                i.setSelected(false);
            }
            if (i.getRoom().isAvailable()) {
                if (orderedRooms.contains(i.getRoom().getMaphong())) {
                    i.setEnabled(false);
                } else {
                    i.setEnabled(true);
                }
    ...
    private void checkOrder_new() {
        ArrayList<Integer> orderedRooms = orderBLL.checkOrder_new(from_model.getValue(), to_model.getValue());
        for (RoomToggle i : roomButtons) {
            if (i.isSelected()) {
                i.setSelected(false);
            }
    ...
    public void updateDeposit() {
        ...
        tongcoc.setText(curencyFormat.format(sum));
    }
    ...
    private void initComponents() {
        ...
        from_to.setLayout(new java.awt.GridLayout());
    ...
    private void cmndFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_cmndFocusLost
        customer = orderBLL.checkIdCard(this.cmnd.getText());
        if (customer != null) {
            this.hotenkh.setText(customer.getHotenkh());
            this.gioitinh.setSelectedIndex(customer.isGioitinh() ? 1 : 0);
            this.diachi.setText(customer.getDiachi());
            this.sdt.setText(customer.getSdt());
        } else {
            customer = null;
        }
    ...
    private void sdtFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_sdtFocusLost
        if (!orderBLL.checkPhone(sdt.getText())) {
            sdt.setText("");
        }
    ...
    private void ngaynhanFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_ngaynhanFocusLost
        ...
        calendar.set(Calendar.SECOND, 0);
        calendar.set(Calendar.MILLISECOND, 0);
        Timestamp checkIn = orderBLL.convert(ngaynhan.getText(), 0);
        if (checkIn.getTime() < calendar.getTime().getTime()) {
            ngaynhan.setText(orderBLL.dateFormat.format(calendar.getTime()));
        } else if (checkIn.getTime() > orderBLL.convert(ngaytra.getText(), 0).getTime()) {
            long tomorrow = checkIn.getTime() + (1000 * 60 * 60 * 24);
            ngaytra.setText(orderBLL.dateFormat.format(new Timestamp(tomorrow)));
        }
        checkOrder();
    ...
    private void ngaytraFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_ngaytraFocusLost
        Timestamp checkOut = orderBLL.convert(ngaytra.getText(), 0);
        long tomorrow = orderBLL.convert(ngaynhan.getText(), 0).getTime() + (1000 * 60 * 60 * 24);
